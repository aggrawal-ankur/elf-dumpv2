...
int main(int argc, char *argv[]){
  ...
  memset(AccessFile, 0, sizeof(ElfFile));           // To make ptrs null and variables 0 in one hit.

  AccessFile->filename = argv[1];
  // printf("\n\n%s\n\n", argv[1]);
  printf("\n\n%s\n\n", AccessFile->filename);

  printf("Parsing file headers....\n");
  if (parse_ehdr(f_obj, AccessFile) != 0){
  ...
  if (parse_phdrs(f_obj, AccessFile) != 0){
  ...
  if (parse_shdrs(f_obj, AccessFile) != 0){
  ...
  if (parse_shstrtab(f_obj, AccessFile) != 0){
  ...
  if (parse_strtab(f_obj, AccessFile) != 0){
  ...
  if (parse_symtab(f_obj, AccessFile) != 0){
  ...
  if (parse_dynsym(f_obj, AccessFile) != 0){
  ...
  if (parse_relocations(f_obj, AccessFile) != 0){
  ...
  if (parse_dynstr(f_obj, AccessFile) != 0){
  ...
  if (parse_dynamic(f_obj, AccessFile) != 0){
